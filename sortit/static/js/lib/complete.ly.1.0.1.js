...
function completely(container, config) {
    ...
    var txtInput = document.getElementById(config.inputId);
    ...
    var createDropDownController = function(elem) {
        ...
        var p = {
            ...
            selected: function() { return rows[ix];},
            refresh : function(token, array) {
                ...
                var loopLength = config.maxResults > -1 ? Math.min(array.length, config.maxResults) : array.length;
                var i, divRow;
                rows = [];
                for (i=0;i<loopLength;i++) {
                    if (array[i].toLowerCase().indexOf(token)===-1) { continue; }
                    divRow = document.createElement('div');
                    divRow.onmouseover = onMouseOver; 
                    divRow.onmouseout =  onMouseOut;
                    divRow.onmousedown = onMouseDown; 
                    divRow.__hint =    array[i];
                    divRow.innerHTML = '<b>'+array[i]+'</b>';
                    divRow.value = array[i];
                    rows.push(divRow);
    ...
    dropDownController.onmouseselection = function(text) {
      txtInput.value = "";
      registerOnTextChangeOldValue = null;
      rs.onSelection(text);
    };
    ...
    var rs = { 
        ...
        repaint : function() {
            ...
            var token = text.substring(startFrom).toLowerCase();
    ...
    **/
    ...
    var keyDownHandler = function(e) {
        ...
        if (keyCode === 13) {       // enter  (autocomplete triggered)
          if(!dropDownController.isVisible()){
            rs.enterWithoutDropDown();
            return;
          }
          dropDownController.hide();
          txtInput.value="";
          registerOnTextChangeOldValue = null;
          return rs.onSelection(dropDownController.selected().value);
        }
        if (keyCode === 40) {     // down
